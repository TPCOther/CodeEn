...
from dataset.atlas_dataset import AtlasDataset, AtlasPrompt, build_atlas_assertion_promp_jarcard, build_atlas_assertion_prompt_bm25, process_query
...
class ModelTester():
    ...
    def metric_2(self, dataset_type):
        ...
        if "bm_catch_neighbor" not in locals():
            ...
            for index in tqdm(range(len(datasets))):
                t = datasets[index]
                num_processes = 16
                chunk_size = math.ceil(len(test_methods) / num_processes)
                templates = [
                    test_methods[i:i + chunk_size]
                    for i in range(0, len(test_methods), chunk_size)
                ]

                with Pool(num_processes) as pool:
                    results = []
                    for i in range(len(templates)):
                        trainFocalTestMethodListSplit = templates[i]
                        results.append(
                            pool.apply_async(process_query,
                                            args=(t.focal_test,
                                                trainFocalTestMethodListSplit)))
                    retrive_results = []
                    for i in range(len(results)):
                        single_result = results[i].get()
                        retrive_results.extend(single_result)
    ...
    def priorization(self, method = "OFTOR"):
        ...
        if method == "OFTOR":
            ...
            valid_hidden_vars = np.load(valid_metric4_file)
            valid_metrics.append(valid_hidden_vars)
            test_hidden_vars = self.metric_4("test", model_under_test, times=10, tokenizer=tokenizer, model=model, device=device)
            test_metrics.append(test_hidden_vars)
...
if __name__ == "__main__":
    ...
    for model_under_test, abstract in map_dic.items(): 
        for demonstration_number in [15]:
        # for BEAM_SIZE in [15]:
            BEAM_SIZE = 10
            result_dir = f"./outputs/ATLAS_LOU/k_{demonstration_number}_m_{BEAM_SIZE}/"

            train_file_path = f"./Dataset/old/{abstract}_{BEAM_SIZE}_train.raw.lou.rank.jsonl"
            valid_file_path = f"./Dataset/old/{abstract}_{BEAM_SIZE}_val.raw.lou.rank.jsonl"
            test_file_name = f"./Dataset/old/{abstract}_{BEAM_SIZE}_test.raw.lou.rank.jsonl"
