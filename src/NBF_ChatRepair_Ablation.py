...
from NBF_prompt_construction import OpenAIClientSingleton, nbf_gen_demo_prompt, nbf_gen_system_prompt, nbf_gen_user_prompt, nbf_get_zero_shot_prompt, nbf_get_zero_shot_prompt_repair, nbf_repair_demo_prompt
...
from dataset.nbf_dataset import NBFDataset, NBFPrompt, build_nbf_promp_jarcard, build_nbf_prompt_bm25, process_query, build_vocab, process_query_vocab
...
class ModelTester():
    ...
    def generate_final_result(self, model, mode, **kwargs):
    ...
    def RFE_Feature_Selection(self):
        ...
        elif self.model_under_test == "Unixcoder":
            BEAM_SIZE = 1
            tokenizer = AutoTokenizer.from_pretrained(
                "./Unixcoder-base/snapshots/727b99cf2a9cab12f417ac638ecd7c9242e896nbf", device_map=device)
            
            config = RobertaConfig.from_pretrained(
                "./Unixcoder-base/snapshots/727b99cf2a9cab12f417ac638ecd7c9242e896nbf"
            )
        ...
        valid_metric1_file = self.result_dir + f"{model_under_test}_valid_metric1.npy"

        valid_metric2_file = self.result_dir + f"{model_under_test}_valid_metric2.npy"

        valid_metric3_file = self.result_dir + f"{model_under_test}_valid_metric3.npy"

        if os.path.exists(valid_metric1_file) == False:
            valid_ppls = self.metric_1("valid")
            np.save(valid_metric1_file, valid_ppls)
        else:
            valid_ppls = np.load(valid_metric1_file)
        

        if os.path.exists(valid_metric2_file) == False:
            valid_neighor_sim = self.metric_2("valid")
            np.save(valid_metric2_file, valid_neighor_sim)
        else:
            valid_neighor_sim = np.load(valid_metric2_file)
        
        if os.path.exists(valid_metric3_file) == False:
            ...
            np.save(valid_metric3_file, valid_sem_sim)

        else:
            valid_sem_sim = np.load(valid_metric3_file)
        
        train_X = np.vstack((valid_ppls, valid_neighor_sim, valid_sem_sim)).T
        ...
        clf = GradientBoostingClassifier(n_estimators=100, learning_rate=1.0,
                                            max_depth=1, random_state=0).fit(train_X, y)
    ...
    def priorization(self, method = "OFTOR", metrics=None):
        '''
        ...
        if method == "OFTOR":
            ...
            if 1 in metrics:
                valid_metric1_file = self.result_dir + f"{model_under_test}_valid_metric1.npy"
                valid_ppls = np.load(valid_metric1_file)
                valid_metrics.append(valid_ppls)

                test_ppls = self.metric_1("test")
                test_metrics.append(test_ppls)
            
            if 2 in metrics:
                valid_metric2_file = self.result_dir + f"{model_under_test}_valid_metric2.npy"
                valid_neighor_sim = np.load(valid_metric2_file)
                valid_metrics.append(valid_neighor_sim)

                test_neighor_sim = self.metric_2("test")
                test_metrics.append(test_neighor_sim)
            
            if 3 in metrics:
                valid_metric3_file = self.result_dir + f"{model_under_test}_valid_metric3.npy"

                hidden_state_file_path = self.result_dir + f"{model_under_test}_train_hidden_states.npy"
                valid_hidden_state_file_path = self.result_dir + f"{model_under_test}_valid_hidden_states.npy"
                test_hidden_state_file_path = self.result_dir + f"{model_under_test}_test_hidden_states.npy"
                ...
                if os.path.exists(valid_hidden_state_file_path) == False:
                    valid_hidden_states = self.get_hidden_state(model, tokenizer, device, "valid", model_under_test, is_dropout=False)
                    np.save(valid_hidden_state_file_path, valid_hidden_states)
                else:
                    valid_hidden_states = np.load(valid_hidden_state_file_path)
                
                if os.path.exists(valid_metric3_file) == True:
                    valid_sem_sim = np.load(valid_metric3_file)
                    valid_metrics.append(valid_sem_sim)
                ...
                if os.path.exists(test_hidden_state_file_path) == False:
                    test_hidden_states = self.get_hidden_state(model, tokenizer, device, "test", model_under_test, is_dropout=False)
                    np.save(test_hidden_state_file_path, test_hidden_states)
                else:
                    test_hidden_states = np.load(test_hidden_state_file_path)

                test_sem_sim = self.metric_3("test", test_hidden_states, train_hidden_states)

                test_metrics.append(test_sem_sim)



            train_X = np.vstack(valid_metrics).T

            test_X = np.vstack(test_metrics).T     
            ...
            wrong_prob= clf.predict_proba(test_X)[:, 1]
            #获取wrong_prob概率大于0.45数据索引
        ...
        return len(sorted_index_desc_select)        
        ...
        for line in tqdm(self.selected_test_data):
            ...
            for i in range(len(candidates)):
                ...
                if Tokenizer.Recoder_whether_equally(
                        expected=gNBFs.strip(), actual=generation.strip()):
                    if i == 0:
                        one_correct += 1
                    else:
                        five_correct += 1
    ...
    def repair(self,
        ...
        if mode == NBF_models.nbf_mode.zero_shot:
            ...
            for line_index, t in tqdm(
                ...
                if line_index in self.catch:
                    continue
                fix_prompt = nbf_get_zero_shot_prompt(bug=t.bug)
                fix_prompts.append(fix_prompt)
                filtered_sorted_index_desc_select.append(line_index)
                filtered_selected_test_data.append(t)
...
def construct_generation_prompt(demonstration_records,inference: 'NBF_models.NBF_Datapoint'):
    ...
    query = nbf_gen_user_prompt(inference.bug)
...
if __name__ == "__main__":
    ...
    BEAM_SIZE = 10
    seed = 42
    result_dir = "./outputs/NBFs/Ablation/"
    ...
    for model_under_test, abstract in map_dic.items():
        ...
        # Step 1: 获得特征的重要性
        ranking = experiment.RFE_Feature_Selection()

        Ablation_results = []

        for i in range(1, len(ranking)+1):
            print("The Selected Features: ", ranking[:i])
            selected_number = experiment.priorization(method="OFTOR", metrics = ranking[:i])
            pre, recall = experiment.get_metrics()
            Ablation_results.append([selected_number, pre, recall, ranking[:i]])            
        ...
        with open(result_dir + f"{model_under_test}_Ablation_results.txt", "w") as writer:
            writer.write("The Feature Importance: " + str(ranking) + "\n")
            writer.write("*"*50 + "\n")
        ...
        # experiment.generate_final_result(model=chat_model, mode=repair_mode)
