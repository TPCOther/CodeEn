...
class ModelTester():
        ...
        with jsonlines.open(output_file_name, mode='w') as writer:
            for t in self.keep_test_data:
                object = {
                    ...
                    "buggy_code": t.bug.split("[eviden ce]")[0].split("[source_code]")[1].strip().split(' '),
    ...
    def gini(self, model_under_test, model, tokenizer, device, select_number):
        outputs = []
        output, seq_len = get_model_output(self.test_set, self.test_objects, model, tokenizer, device, model_under_test, task="NBF")
    ...
    def BADGE_sample(self, hidden_state, model_under_test, model, tokenizer, device, select_number):
        ...
        selected_index = sampling_module.select_batch_to_label(np.array([], dtype=np.int64), select_number, task = "NBF")
    ...
    def priorization(self, method = "OFTOR"):
        ...
        if method != "PPL":
            ...
            if self.model_under_test == "CodeT5":
                select_num = 8670
                tokenizer = AutoTokenizer.from_pretrained("./outputs/NBFs/CodeT5/checkpoint-2324", device_map=device)
                model = T5ForConditionalGeneration.from_pretrained("./outputs/NBFs/CodeT5/checkpoint-2324").to(device)

            elif self.model_under_test == 'CodeGen':
                select_num = 8187
                tokenizer = AutoTokenizer.from_pretrained("./outputs/NBFs/CodeGen/checkpoint-9293",
                                                        padding_side="left", device_map=device)
                model = AutoModelForCausalLM.from_pretrained("./outputs/NBFs/CodeGen/checkpoint-9293").to(device)
                ...
                model.load_state_dict(torch.load(os.path.join("./outputs/NBFs/Unixcoder/checkpoint-0/pytorch_model.bin")),
                                    strict=False)
                model.to(device)
            ...
            if method == "ori":
                with priorization_timer:
                    output, seq_len, sequences = get_model_output(self.test_set, self.test_objects, model, tokenizer, device, model_under_test, return_sequences=True, task="NBF")
    ...
    def repair(self,
        ...
        if mode in [NBF_models.nbf_mode.bm_25, NBF_models.nbf_mode.jarcard]:
            ...
            for datapoint in tqdm(self.training_data):
                # if model != ("gpt-3.5-turbo" or "gpt-4o-2024-08-06"):
...
if __name__ == "__main__":
    ...
    argparser.add_argument("--method", type=str, default="PPL")
    ...
    BEAM_SIZE = 10
    seed = 42
    result_dir = "./outputs/NBFs_baseline/"
    setseed(seed)
    ...
    for model_under_test, abstract in map_dic.items():
        for method in ["ori", "kcenter", "gini", "badge"]:
