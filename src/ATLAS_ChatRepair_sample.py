...
import ATLAS_models
from ATLAS_prompt_construction import OpenAIClientSingleton, assertion_gen_demo_prompt, assertion_gen_system_prompt, assertion_gen_user_prompt, assertion_get_whether_repair_prompt, assertion_get_zero_shot_prompt, assertion_get_zero_shot_prompt_repair, assertion_repair_demo_prompt, vLLMClientSingleton
...
import evaluate
from Sample import get_model_output, BADGESampling, calculate_autoregressive_ppl

#time counter
timer = AccumulatedTimer()
repair_timer = AccumulatedTimer()
priorization_timer = AccumulatedTimer()
valid_timer = AccumulatedTimer()
...
def get_osmodel_result(prompt, line_indexs, templates, mode, model, n, temperture,
                       ts: ATLAS_models.ATLAS_Datapoint):
    results = []
...
class ModelTester():
    ...
    def generate_final_result(self, model, mode, prediction_mode = "combined", method = None, **kwargs):
        '''
        ...
        if method is not None:
            output_file_name = output_file_name.replace("v2", f"{method}")
    ...
    def metric_3(self, dataset_type, hidden_states, train_hidden_states):
        ...
        import torch.nn.functional as F
        y = torch.tensor(train_hidden_states, dtype=torch.float32).unsqueeze(0).to("cuda:0")
        chuck = 5000
    ...
    def kcentergreedy(self, hidden_states, train_hidden_states, select_number):
        hidden_states = torch.tensor(hidden_states)
        train_hidden_states = torch.tensor(train_hidden_states)
        
        print(hidden_states.shape, train_hidden_states.shape)
        ori_labeled_idxs = np.zeros(len(self.test_data), dtype=bool)

        ori_labeled_idxs[:] = False

        chunk_size = 1000

        dist_mat_un_un = torch.zeros(len(self.test_data), len(self.test_data))
        for i in range(0, len(self.test_data), chunk_size):
            batch_embeddings = hidden_states[i:i + chunk_size]
            dist_mat_un_un[i:i + chunk_size, :] = torch.cdist(batch_embeddings, hidden_states, p=2)

        dist_mat_un_label = torch.zeros(hidden_states.shape[0], train_hidden_states.shape[0])
        print(dist_mat_un_label.shape)
        for i in range(0, len(self.test_data), chunk_size):
            batch_embeddings = hidden_states[i:i + chunk_size]
            dist_mat_un_label[i:i + chunk_size, :] = torch.cdist(batch_embeddings, train_hidden_states, p=2)

        mat_min, _ = torch.min(dist_mat_un_label, dim=1, keepdim=True)

        ori_labeled_idxs = torch.BoolTensor(ori_labeled_idxs)

        rank_list = []
        for _ in tqdm(range(select_number)):
            q_idx_ = torch.argmax(mat_min, dim=0)
            q_idx = torch.where(~ori_labeled_idxs)[0][q_idx_]
            ori_labeled_idxs[q_idx] = True
            rank_list.append(q_idx.item())
            mat_min = torch.cat([mat_min[:q_idx_, :], mat_min[q_idx_ + 1:, :]], dim=0)
            mat_min = torch.cat([mat_min, dist_mat_un_un[~ori_labeled_idxs, q_idx].unsqueeze(1)], dim=1)
            mat_min, q_idx_ = torch.min(mat_min, dim=1, keepdim=True)

        return np.array(rank_list)
    
    def gini(self, model_under_test, model, tokenizer, device, select_number):
        outputs = []
        output, seq_len = get_model_output(self.test_set, self.test_objects, model, tokenizer, device, model_under_test, task="ATLAS")
        for o, seql in zip(output, seq_len):
            o = o ** 2
            print(len(o), len(seql))
            for j in range(o.shape[0]):
                osum = 1 - o[j][:seql[j]].sum(dim=-1)
                outputs.append(osum[0].item())
        outputs = np.array(outputs)
        indexs = np.argsort(outputs) 
        return indexs[-select_number:]
    
    def BADGE_sample(self, hidden_state, model_under_test, model, tokenizer, device, select_number, task = None):
        sampling_module = BADGESampling(
            ensemble_models=[model],
            features= hidden_state,
            n=len(self.test_set),
            tset = self.test_set,
            tobject = self.test_objects,
            tokenizer = tokenizer,
            model_type = model_under_test,
            device = device
        )
        
        selected_index = sampling_module.select_batch_to_label(np.array([], dtype=np.int64), select_number, task=task)
    ...
    def priorization(self, method = "OFTOR"):
        ...
        if method != "PPL":
            ...
            if self.model_under_test == "CodeT5":
                select_num = 7070
                tokenizer = AutoTokenizer.from_pretrained("./outputs/ATLAS_LOU/CodeT5/checkpoint-15676", device_map=device)
                model = T5ForConditionalGeneration.from_pretrained("./outputs/ATLAS_LOU/CodeT5/checkpoint-15676").to(device)

            elif self.model_under_test == 'CodeGen':
                select_num = 9654
                tokenizer = AutoTokenizer.from_pretrained("./outputs/ATLAS_LOU/CodeGen/checkpoint-47028",
                                                        padding_side="left", device_map=device)
                model = AutoModelForCausalLM.from_pretrained("./outputs/ATLAS_LOU/CodeGen/checkpoint-47028").to(device)
                ...
                model.load_state_dict(torch.load(os.path.join("./outputs/ATLAS_LOU/Unixcoder/checkpoint-5/pytorch_model.bin")),
                                    strict=False)
                model.to(device)
            ...
            sorted_index_desc_select_path = self.result_dir + f"{model_under_test}_sorted_index_desc_select_temp.npy"
            
            if os.path.exists(hidden_state_file_path) == False and method in ["kcenter"]:
                with priorization_timer:
                ...
                print(f"time of getting train hidden states: {priorization_timer.get_total_time()}")
            ...
            print(f"time of getting test hidden states: {priorization_timer.get_total_time()}")
            
            if method == "kcenter":
                if(os.path.exists(sorted_index_desc_select_path.replace("temp", "kcenter")) == False):
                    with priorization_timer:
                        sorted_index_desc_select = self.kcentergreedy(test_hidden_states, train_hidden_states, select_num)
                    np.save(sorted_index_desc_select_path.replace("temp", "kcenter"), sorted_index_desc_select)
                else:
                    sorted_index_desc_select = np.load(sorted_index_desc_select_path.replace("temp", "kcenter"))
            
            if method == "gini":
                if(os.path.exists(sorted_index_desc_select_path.replace("temp", "gini")) == False):
                    with priorization_timer:
                        sorted_index_desc_select = self.gini(model_under_test, model, tokenizer, device, select_num)
                    np.save(sorted_index_desc_select_path.replace("temp", "gini"), sorted_index_desc_select,)
                else:
                    sorted_index_desc_select = np.load(sorted_index_desc_select_path.replace("temp", "gini"))
                
            if method == "badge":
                if(os.path.exists(sorted_index_desc_select_path.replace("temp", "badge")) == False):
                    with priorization_timer:
                        sorted_index_desc_select = self.BADGE_sample(test_hidden_states, model_under_test, model, tokenizer, device, select_num, task = "ATLAS")
                    np.save(sorted_index_desc_select_path.replace("temp", "badge"), sorted_index_desc_select)
                else:
                    sorted_index_desc_select = np.load(sorted_index_desc_select_path.replace("temp", "badge"))

            if method == "ori":
                with priorization_timer:
                    output, seq_len, sequences = get_model_output(self.test_set, self.test_objects, model, tokenizer, device, model_under_test, return_sequences=True, task="ATLAS")
                    ppls = calculate_autoregressive_ppl(output, seq_len, sequences)
                with valid_timer:
                    valid_hidden_states = self.get_hidden_state(model, tokenizer, device, "valid", model_under_test, is_dropout=False)
                
                valid_hidden_states = valid_hidden_states.reshape(-1, valid_hidden_states.shape[-1])
                print(f"time of getting valid hidden states: {valid_timer.get_total_time()}")

                np.save(hidden_state_file_path, valid_hidden_states)
                sorted_index_desc_select = [0]
                    
                    
                
            print(f"Finish prioritization {method}, the selected number is {len(sorted_index_desc_select)}")
            print(sorted_index_desc_select)
    ...
    def get_metrics(self):
        ...
        print("len g p: ", len(ground_truth), len(prediction))
        print("sum:", sum(ground_truth), sum(all_ground_truth))
...
if __name__ == "__main__":
    ...
    demonstration_number = 5
    BEAM_SIZE = 10
    seed = 42
    result_dir = "./outputs/ATLAS_LOU_baseline/"
    # map_dic = {"Unixcoder": "unix", "CodeGen": "codegen"}
    # map_dic = {"CodeGen": "codegen"}
    ...
    for model_under_test, abstract in map_dic.items():
        for method in ["gini", "kcenter", "badge"]:    
            ...
            selected_number = experiment.priorization(method=method)
            print("="*50, method, "="*50)
            print(f"priorization time: {priorization_timer.get_total_time()}")
            print(f"valid time: {valid_timer.get_total_time()}")

            pre, recall = experiment.get_metrics()
            print("pre: ", pre)
            print("recall: ", recall)
            Ablation_results.append([selected_number, pre, recall, method])
            # continue
            ...
            for prediction_model in ["combined"]:
                experiment.generate_final_result(model=chat_model, mode=repair_mode, prediction_mode=prediction_model, method=method)



            print(f"repair_timer: {repair_timer.get_total_time()} seconds, priorization_timer: {priorization_timer.get_total_time()} seconds")
            print(f"Total accumulated time: {timer.get_total_time() + repair_timer.get_total_time()} seconds")
            valid_timer.reset()
            priorization_timer.reset()
            
        with open(result_dir + f"{model_under_test}_Ablation_results.txt", "a") as writer:
            writer.write("The Feature Importance: " + model_under_test + "\n")
            writer.write("*"*50 + "\n")
            for result in Ablation_results:
                writer.write("The Selected Features: " + str(result[3]) + "\n")
                writer.write("The Selected Number: " + str(result[0]) + "\n")
                writer.write("The Precision: " + str(result[1]) + "\n")
                writer.write("The Recall: " + str(result[2]) + "\n")
                writer.write("The F1: " + str(2*result[1]*result[2]/(result[1]+result[2])) + "\n")
                writer.write("*"*50 + "\n")
