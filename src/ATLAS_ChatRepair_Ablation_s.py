    ...
    def priorization(self, method = "OFTOR", classifier = "GBDT", ablation = "ALL"):
        ...
        if method == "OFTOR":
            ...
            if ablation == "PPL":
                ...
                TX = np.vstack(test_ppls)
            elif ablation == "STOS":
                X = np.vstack(valid_neighor_sim)
                TX = np.vstack(test_neighor_sim)
            elif ablation == "SMOS":
                X = np.vstack(valid_sem_sim)
                TX = np.vstack(test_sem_sim)
            elif ablation == "ALL":
                X = np.vstack((valid_ppls, valid_neighor_sim, valid_sem_sim)).T
                TX = np.vstack((test_ppls, test_neighor_sim, test_sem_sim)).T
            ...
            with timer:
                wrong_prob= clf.predict_proba(TX)[:, 1]
...
if __name__ == "__main__":
    ...
    BEAM_SIZE = 10
    seed = 42
    result_dir = "./outputs/ATLAS_LOU_classifier/"
    # map_dic = {"CodeT5": "t5"}
    ...
    for model_under_test, abstract in map_dic.items(): 
        for ablation in ["SVM", "RF", "LR", "GBDT"]:
            ...
            for prediction_model in ["combined"]:
                experiment.generate_final_result(model=chat_model, mode=repair_mode, prediction_mode=prediction_model, method=ablation)
        ...
        with open(result_dir + f"classifier_{model_under_test}_Ablation_results.txt", "w") as writer:
