...
class ModelTester():
    ...
    def metric_2(self, dataset_type, weight=True):
        '''
        根据相似度来计算比较相应生成的相似度
        '''
    ...
    def metric_3(self, dataset_type, hidden_states, train_hidden_states, weight=True):
        ...
        for index, t in tqdm(enumerate(datasets)):
            ...
            if weight:
                ...
                seman_sim_scores.append(nl_not_sim)
    ...
    def priorization(self, method = "OFTOR", weight=True):
        '''
        ...
        if method == "OFTOR":
            ...
            valid_neighor_sim = self.metric_2("valid", weight=weight)

            with timer:
                test_neighor_sim = self.metric_2("test", weight=weight)            
            ...
            valid_sem_sim = self.metric_3("valid", valid_hidden_states, train_hidden_states, weight=weight)
            ...
            with timer:
                ...
                test_sem_sim = self.metric_3("test", test_hidden_states, train_hidden_states, weight=weight)
...
if __name__ == "__main__":
    ...
    for model_under_test, abstract in map_dic.items():
        ...
        selected_number = experiment.priorization(method="OFTOR", weight=False)
        ...
        print("pre: ", pre)
        print("recall: ", recall)
        Ablation_results.append([selected_number, pre, recall, model_under_test])
        ...
        for prediction_model in ["combined"]:
            experiment.generate_final_result(model=chat_model, mode=repair_mode, prediction_mode=prediction_model, method="no_weight")
    ...
    with open(result_dir + f"Ablation_weight_s_results.txt", "w") as writer:
