...
class ModelTester():
    ...
    def generate_final_result(self, model, mode, prediction_mode = "combined", k=None, **kwargs):
        ...
        if k is not None:
            output_file_name = output_file_name.replace("v2", str(k))
    ...
    def metric_2(self, dataset_type, k=500):
        '''
        根据相似度来计算比较相应生成的相似度
        '''
        ...
        if dataset_type == "valid":
            bm_catch_name = self.result_dir + f"{dataset_type}_jarcard_neighbor_metric3_{k}.jsonl"
            ...
            bm_catch_name = self.result_dir + f"jarcard_neighbor_{k}.jsonl"
            if os.path.exists(bm_catch_name) == True:
                bm_catch_neighbor = []
    ...
    def metric_3(self, dataset_type, hidden_states, train_hidden_states):
        ...
        import torch.nn.functional as F
        y = torch.tensor(train_hidden_states, dtype=torch.float32).unsqueeze(0).to("cuda:2")
        chuck = 5000
    ...
    def priorization(self, method = "OFTOR", k=500):
        '''
        ...
        if method == "OFTOR":
            ...
            device = torch.device(
            f"cuda:{2}" if torch.cuda.is_available() else "cpu")
            ...
            valid_neighor_sim = self.metric_2("valid", k)

            with timer:
                test_neighor_sim = self.metric_2("test", k)            
            ...
            timer.reset()
            with timer:
                test_ppls = self.metric_2("test", k=k)
            
            print("Time taken to calculate metrics: ", timer.get_total_time())
            return True
...
if __name__ == "__main__":
    ...
    for model_under_test, abstract in map_dic.items():
        for k in [20, 100, 250, 500, 1000]:
            ...
            experiment.priorization(method="PPL", k=k)
            continue
            ...
            for prediction_model in ["combined"]:
                experiment.generate_final_result(model=chat_model, mode=repair_mode, prediction_mode=prediction_model, k=k)
