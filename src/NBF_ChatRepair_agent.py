...
from NBF_prompt_construction import OpenAIClientSingleton, vLLMClientSingleton, nbf_gen_demo_prompt, nbf_gen_system_prompt, nbf_gen_user_prompt, nbf_get_zero_shot_prompt, nbf_get_zero_shot_prompt_repair, nbf_repair_demo_prompt, check_nbf_prompt, regen_demo_nbf_prompt
...
def agent_loop(prompt, line_indexs, templates, mode, model, n, temperture, ts: NBF_models.NBF_Datapoint):
    ...
    while k < max_repair:
        ...
        check_prompt = check_nbf_prompt(code_target, pred_target)
        responses = vLLMClientSingleton.get_response(
        ...
        fix_prompt = regen_demo_nbf_prompt(prompt_target, pred_target)
    ...
    for model_under_test, abstract in map_dic.items():
        ...
        with repair_timer:
            experiment.repair(model=chat_model,
                            ...
                            with_commands=True)
        end_time = time.time()
        print("ori priorization time", priorization_time)
        print("ori repair time", end_time - start_time)
