    ...
    def priorization(self, method = "OFTOR", classifier="GBDT", ablation="ALL"):
        ...
        if method == "OFTOR":
            ...
            if ablation == "ALL":
                ...
                XT = test_ppls.reshape(-1, 1)
            elif ablation == "STOS":
                X = valid_neighor_sim.reshape(-1, 1)
                XT = test_neighor_sim.reshape(-1, 1)
            elif ablation == "SMOS":
                X = valid_sem_sim.reshape(-1, 1)
                XT = test_sem_sim.reshape(-1, 1)
            print(X.shape)
...
if __name__ == "__main__":
    ...
    BEAM_SIZE = 10
    seed = 42
    result_dir = "./outputs/NBFs_classifier/"
    setseed(seed)
