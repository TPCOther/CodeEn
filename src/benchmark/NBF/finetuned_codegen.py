...
class DecoderTrainer(Trainer):
    def evaluate(self, eval_dataset=None, ignore_keys=None, metric_key_prefix: str = "eval"):
        ...
        for i in tqdm(range(0, len(eval_dataset), batch_size)):
            input_data = [generate_prompt(INS, input) for input in eval_dataset[i:i+batch_size]["bug"]]
            ...
            with torch.no_grad():
                generated_ids = self.model.generate(model_inputs.input_ids, max_new_tokens=256, generation_config=generation_config)
        ...
        metric = self.compute_metrics({'predictions': predictions, 'labels': [[label] for label in eval_dataset["fix"]]})
        metrics = {}
...
class DataArguments:
    ...
    add_path: str = field(default=None, metadata={"help": "Path to the additional training data."})
...
def train_tokenize_function(examples, tokenizer):
    ...
    if 'bug' in examples:
        sources = [
            ...
            for input in examples['bug']
        ]
    else:
        sources = [ prompt_no_input.format_map(dict(instruction=INS)) ]
    targets = [f"{output}{tokenizer.eos_token}" for output in examples['fix']]
...
if __name__ == "__main__":
    '''
    CUDA_VISIBLE_DEVICES=0 python /data/swf/Assertion/src/benchmark/BF/finetuned_codegen.py \
    --model_name_or_path "/data/swf/Assertion/codegen-base" \
    --data_path "Dataset/BFs/train.json" \
    --valid_path "Dataset/BFs/valid.json" \
    --output_dir "/data/swf/Assertion/outputs/BFs/CodeGen" \
    --num_train_epochs 10 \
