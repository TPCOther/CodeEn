...
def read_examples(filename, add_filename=None):
    ...
    with open(filename,encoding="utf-8") as f:
        ...
        if add_filename != None:
            with open(add_filename,encoding="utf-8") as f:
                for idx, line in enumerate(f):
                    line = line.strip()
                    js = json.loads(line)
                    code = js['code'].replace('\n',' ').strip()
...
def main():
    ...
    parser.add_argument("--add_filename", default=None, type=str)
    ...
    if args.do_train:
        if(args.model_path != None):
            logger.info("Loading checkpoint from {}".format(args.model_path))
            model_to_load = model.module if hasattr(model, 'module') else model  
            model_to_load.load_state_dict(torch.load(args.model_path))  
        # Prepare training data loader
        train_examples = read_examples(args.train_filename, args.add_filename)
    ...
    if args.do_test:
        ...
        model_to_load = model.module if hasattr(model, 'module') else model  
        logger.info("Loading checkpoint from {}".format(output_dir))
...
if __name__ == "__main__":
    '''
    # Training
    CUDA_VISIBLE_DEVICES=2 python /data/swf/Assertion/src/benchmark/JCSD/UnixCoder_run.py \
	--do_train \
	--do_eval \
	--model_name_or_path /data/swf/Assertion/Unixcoder-base/snapshots/727b99cf2a9cab12f417ac638ecd7c9242e896bf \
	--train_filename Dataset/JCSD/train.json \
	--dev_filename Dataset/JCSD/valid.json \
	--output_dir /data/swf/Assertion/outputs/JCSD/Unixcoder \
