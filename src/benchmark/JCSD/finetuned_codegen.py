...
class DecoderTrainer(Trainer):
    def evaluate(self, eval_dataset=None, ignore_keys=None, metric_key_prefix: str = "eval"):
        ...
        for i in tqdm(range(0, len(eval_dataset), batch_size)):
            input_data = [generate_prompt(INS, input) for input in eval_dataset[i:i+batch_size]["code"]]
        ...
        metric = self.compute_metrics({'predictions': predictions, 'labels': [[label] for label in eval_dataset["comment"]]})
        metrics = {}
...
def train_tokenize_function(examples, tokenizer):
    ...
    if 'code' in examples:
        sources = [
            ...
            for input in examples['code']
        ]
    else:
        sources = [ prompt_no_input.format_map(dict(instruction=INS)) ]
    targets = [f"{output}{tokenizer.eos_token}" for output in examples['comment']]
...
if __name__ == "__main__":
    '''
    CUDA_VISIBLE_DEVICES=3 python /data/swf/Assertion/src/benchmark/JCSD/finetuned_codegen.py \
    --model_name_or_path "/data/swf/Assertion/codegen-base" \
    --data_path "Dataset/JCSD/train.json" \
    --valid_path "Dataset/JCSD/valid.json" \
    --output_dir "/data/swf/Assertion/outputs/JCSD/CodeGen" \
    --num_train_epochs 10 \
