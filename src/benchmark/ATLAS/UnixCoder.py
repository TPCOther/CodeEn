...
class GenerateOutput:
    def __init__(self, seq=None, logits=None):
        self.sequences = seq
        self.scores = logits
...
class Seq2Seq(nn.Module):
    ...
    def forward(self, source_ids, target_ids=None, return_logits=False):
        if target_ids is None:
            pred, logits = self.generate(source_ids)
            if return_logits:
                return GenerateOutput(pred, logits)
            else:
                return pred
    ...
    def generate(self, source_ids):
        ...
        for i in range(source_ids.shape[0]):
            ...
            for _ in range(self.max_length): 
                ...
                self.hidden = out
                hidden_states = out[:,-1,:]
                logits = self.lm_head(hidden_states)  # 这里获取 logits
                logits_for_single_input.append(logits.unsqueeze(1))
                out = self.lsm(logits).data
                beam.advance(out)
            ...
            all_logits.append(torch.cat(logits_for_single_input, dim=1))

        preds = torch.cat(preds,0)
        max_seq_length = max(logits.shape[1] for logits in all_logits)

    # 对每个序列的 logits 进行填充
        padded_logits = []
        for logits in all_logits:
            padding_length = max_seq_length - logits.shape[1]
            if padding_length > 0:
                padding = torch.zeros(logits.shape[0], padding_length, logits.shape[2], device=logits.device)
                logits_padded = torch.cat([logits, padding], dim=1)
            else:
                logits_padded = logits
            padded_logits.append(logits_padded)

        return preds, torch.cat(padded_logits, dim=0) 
