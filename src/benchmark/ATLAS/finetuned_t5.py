...
parser.add_argument("--output_dir", type=str, default="./outputs/ATLAS_LOU/CodeT5")
...
def preprocess_example(example):
    ...
    # inputs = [prefix + focal_test.replace("\n", " ").strip() for focal_test in focal_tests]
    # assertions = [assertion.replace("\n", " ").strip() for assertion in assertions]
...
def comput_metrics(eval_pred):
    ...
    result = exact_match_metric.compute(predictions=decode_preds, references=decode_labels)
