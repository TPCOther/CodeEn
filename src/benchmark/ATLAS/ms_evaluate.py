...
def read_examples_(dataset: dict):
    ...
    for idx, obj in enumerate(dataset):
        code, nl = obj['focal_test'], obj['gold']
        code = code.replace('\n', ' ').strip()
        nl = nl.replace('\n', '').strip()
...
def read_examples(dataset: dict):
    """Read examples from filename."""
    examples = []
    code = dataset['focal_test']
    nl = dataset['gold']
    assert len(code) == len(nl)
    for idx, obj in enumerate(zip(code, nl)):
        code, nl = obj
        code = code.replace('\n', ' ').strip()
        nl = nl.replace('\n', '').strip()
        examples.append(Example(
...
def main(model_name, cuda, mode: str, BEAM_SIZE=5, model_type="t5"):
    ...
    if model_type in ["t5", "unix", "plbart"]:
        batch_size = 8
    ...
    config = {
        'train': "Dataset/ATLAS_LOU/train.raw.lou.jsonl",
        'test': "Dataset/ATLAS_LOU/test.raw.lou.jsonl",
        'valid': "Dataset/ATLAS_LOU/val.raw.lou.jsonl",
    }
    ...
    for i in tqdm(range(0, len(dataset), batch_size)):
        if model_type == "t5":
            ...
            input_data = [
                ...
                for input in dataset[i:i + batch_size]["focal_test"]
            ]
    ...
    if mode == "train":
        ...
        for i, item in enumerate(
                zip(dataset[:]["gold"], object_lists, ppl, only)):
...
if __name__ == "__main__":
    ...
    # main(
    #     model_name=
    #     "/data/swf/Assertion/outputs/ATLAS_LOU/Unixcoder/checkpoint-5/pytorch_model.bin",
    #     cuda=0,
    #     mode="valid",
    #     model_type="unix",
    #     BEAM_SIZE=10)

    main(model_name="/home/yanmeng/huangnaiqi/Assertion/outputs/ATLAS_LOU/CodeT5/checkpoint-15676",
         cuda=0,
         mode="test",
         model_type="t5",
         BEAM_SIZE=10)
    ...
    # main(
    # model_name=
    # "/data/swf/Assertion/outputs/ATLAS_LOU/CodeGen/checkpoint-47028",
    # cuda=0,
    # mode="test",
    # model_type="codegen",
    # BEAM_SIZE=10)
