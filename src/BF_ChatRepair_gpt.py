...
from BF_prompt_construction import OpenAIClientSingleton, vLLMClientSingleton, bf_gen_demo_prompt, bf_gen_system_prompt, bf_gen_user_prompt, bf_get_zero_shot_prompt, bf_get_zero_shot_prompt_repair, bf_repair_demo_prompt
from benchmark.BF.UnixCoder import Seq2Seq
from benchmark.BF.finetuned_codegen import INS, generate_prompt
from benchmark.BF.ms_evaluate import read_examples_, unix_convert_examples_to_features
from config import MAX_CONCODE_CONTEXT_WINDOW
from dataset.bf_dataset import BFDataset, BFPrompt, build_bf_promp_jarcard, build_bf_prompt_bm25, process_query, build_vocab, process_query_vocab
...
def get_gpt_result(prompt, line_index, templates, mode, model, n, temperture,
    ...
    if len(repaired_fix) == 0:
        ...
        if mode == BF_models.bf_mode.zero_shot:
...
class ModelTester():
    ...
    def __init__(self, train_file_path, valid_file_path, test_file_name,
        ...
        #加载各类数据
        self.training_set = BFDataset(self.train_file_path, mode="train")
        self.test_set = BFDataset(self.test_file_name, mode="test")
        self.valid_set = BFDataset(self.valid_file_path, mode="valid")
        ...
        self.training_data: list[
            BF_models.BF_Datapoint] = self.training_set.parse()
        self.test_data: list[BF_models.BF_Datapoint] = self.test_set.parse()
        self.valid_data: list[BF_models.BF_Datapoint] = self.valid_set.parse()
    ...
    def generate_final_result(self, model, mode, prediction_mode = "combined", **kwargs):
        ...
        # model = model.split("/")[1]
        if mode in [
                BF_models.bf_mode.zero_shot,
                BF_models.bf_mode.zero_shot_repair, 
                BF_models.bf_mode.bm_25,
                BF_models.bf_mode.jarcard
        ]:
        ...
        if mode in [
            ...
            with jsonlines.open(output_file_name, mode='a') as writer:
                for index, t in zip(self.sorted_index_desc_select,
                    ...
                    save_object["BFs_generation"] = t.prediction
    ...
    def read_catch(self, model, mode, **kwargs):
        ...
        if mode in [
                BF_models.bf_mode.zero_shot,
                BF_models.bf_mode.zero_shot_repair, BF_models.bf_mode.bm_25,
                BF_models.bf_mode.jarcard
        ]:
    ...
    def metric_2(self, dataset_type):
        ...
        for dp in self.training_data:
            ...
            # tokenized_test_method = dp.datapoint.test_method.split(" ")
            tokenized_test_method = dp.bug.split(" ")
        ...
        if "bm_catch_neighbor" not in locals():
            ...
            for index in tqdm(range(len(datasets))):
                t = datasets[index]
                retrive_results = process_query_vocab(vectorizer, candidate_set, t.bug, md5_hash)
                ...
                filtered_query = [jarcard_cache_dict[md5].bug for md5 in results_top_n]
                top_results = process_query(t.bug, filtered_query)
        ...
        for t in tqdm(datasets):
            ...
            bug_sim = utils.getJaccard(t.bug.strip().split(' '),
                                            jarcard_cache_dict[retrive_results[0]].bug.strip().split(' '))
    ...
    def priorization(self, method = "OFTOR"):
        ...
        if method == "OFTOR":
            ...
            if self.model_under_test == "CodeT5":
                tokenizer = AutoTokenizer.from_pretrained("./outputs/BFs/CodeT5/checkpoint-5836", device_map=device)
                model = T5ForConditionalGeneration.from_pretrained("./outputs/BFs/CodeT5/checkpoint-5836").to(device)

            elif self.model_under_test == 'CodeGen':
                tokenizer = AutoTokenizer.from_pretrained("./outputs/BFs/CodeGen/checkpoint-17505",
                                                        padding_side="left", device_map=device)
                model = AutoModelForCausalLM.from_pretrained("./outputs/BFs/CodeGen/checkpoint-17505").to(device)
                ...
                model.load_state_dict(torch.load(os.path.join("./outputs/BFs/Unixcoder/checkpoint-2/pytorch_model.bin")),
                                    strict=False)
                model.to(device)
    ...
    def get_metrics(self):
        ...
        for line in tqdm(self.selected_test_data):
            ...
            for i in range(len(candidates)):
                ...
                if Tokenizer.Recoder_whether_equally(
                        expected=gBFs.strip(), actual=generation.strip()):
                    if i == 0:
                        one_correct += 1
                    else:
                        five_correct += 1
    ...
    #                 temperture=0,
    #                 n=1,
    #                 mode=BF_models.bf_mode.zero_shot,
    #                 rank=True,
    #                 **kwargs):
    #     '''
    ...
    #     templates = {
    #         BF_models.bf_mode.zero_shot: ("<Generation>", "</Generation>"),
    #         BF_models.bf_mode.zero_shot_repair: ("<Repair>", "</Repair>"),
    #         BF_models.bf_mode.bm_25: ("<Repair>", "</Repair>"),
    #         BF_models.bf_mode.jarcard: ("<Repair>", "</Repair>")
    #     }
    #     batch_thread = 16
    ...
    def repair(self,
        ...
        if mode in [BF_models.bf_mode.bm_25, BF_models.bf_mode.jarcard]:
            ...
            training_data_with_length: list[
                BF_models.bf_datapoint_with_demo_length] = []
            ...
            if mode == BF_models.bf_mode.bm_25:
                print("Loading BM25 ...")
                bm_25_cache_dict = {}
                test_methods = []
            ...
            elif mode == BF_models.bf_mode.jarcard:
            ...
            for line_index, t in tqdm(
                ...
                if mode == BF_models.bf_mode.bm_25:
                    ap: BFPrompt = build_bf_prompt_bm25(
                    ...
                    ap: BFPrompt = build_bf_promp_jarcard(
                        line_index=line_index,
                        bm_catch_neighbor=bm_catch_neighbor,
                        training_data=self.training_data,
                        test_methods=test_methods,
        ...
        if mode == BF_models.bf_mode.zero_shot:
            ...
            for line_index, t in tqdm(
                ...
                if line_index in self.catch:
                    continue
                fix_prompt = bf_get_zero_shot_prompt(bug=t.bug)
                fix_prompts.append(fix_prompt)
                filtered_sorted_index_desc_select.append(line_index)
                filtered_selected_test_data.append(t)

        if mode == BF_models.bf_mode.zero_shot_repair:
            print("Starting constructing prompts ...")
            for line_index, t in tqdm(
                    zip(self.sorted_index_desc_select,
                        self.selected_test_data)):
                if line_index in self.catch:
                    continue
                fix_prompt = bf_get_zero_shot_prompt_repair(
                    bug=t.bug, candidates=t.candidate)
                fix_prompts.append(fix_prompt)
                filtered_sorted_index_desc_select.append(line_index)
                filtered_selected_test_data.append(t)
...
def construct_generation_prompt(demonstration_records,inference: 'BF_models.BF_Datapoint'):

    system_prompt = bf_gen_system_prompt()
    ...
    for i in range(len(demonstration_records) - 1, -1, -1):
        ...
        demo = bf_gen_demo_prompt(record, True)
        if record.bug != '':
            demonstrations.append(demo)

    query = bf_gen_user_prompt(inference.bug)
...
if __name__ == "__main__":
    ...
    repair_mode = BF_models.bf_mode.jarcard
    demonstration_number = 5
    BEAM_SIZE = 10
    seed = 42
    result_dir = "./outputs/BFs/PPL+repair/"
    setseed(seed)
    # map_dic = {"CodeT5": "t5", "Unixcoder": "unix", "CodeGen": "codegen"}
    map_dic = {"CodeGen": "codegen"}
    # map_dic = { "CodeT5": "t5", "Unixcoder": "unix"}
    # client = vLLMClientSingleton(chat_model)

    for model_under_test, abstract in map_dic.items():
        train_file_path = f"./Dataset/BFs/{abstract}_{BEAM_SIZE}_train.json.rank.jsonl"
        valid_file_path = f"./Dataset/BFs/{abstract}_{BEAM_SIZE}_valid.json.rank.jsonl"
        test_file_name = f"./Dataset/BFs/{abstract}_{BEAM_SIZE}_test.json.rank.jsonl"
