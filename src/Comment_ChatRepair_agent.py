...
from Comment_prompt_construction import OpenAIClientSingleton, comment_gen_demo_prompt, comment_gen_system_prompt, comment_gen_user_prompt, comment_get_zero_shot_prompt, comment_get_zero_shot_prompt_repair, comment_repair_demo_prompt, check_comment_prompt, regen_demo_comment_prompt, vLLMClientSingleton
...
def get_osmodel_result(prompt, line_indexs, templates, mode, model, n, temperture,
    ...
    for response, t, line_index, p in zip(responses, ts, line_indexs, prompt):
        ...
        result = {
            ...
            "best_ppl": best_ppl,
            "fix_times": 0,
            "token_usage": len(response.prompt_token_ids)
        }
        results.append(result)

    return results, line_indexs

def agent_loop(prompt, line_indexs, templates, mode, model, n, temperture, ts: Comment_Datapoint):
    results, line_indexs = get_osmodel_result(prompt, line_indexs, templates, mode, model, n, temperture, ts)
    iter_prompt = [r["prompt"] for r in results]
    iter_code = [r["prompt"][-1] for r in results]
    iter_pred = [r["best_repair"] for r in results]
    token_usage = [r["token_usage"] for r in results]
    check_results = [True] * len(results)
    fix_times = [0] * len(results)
    max_repair = 5
    k = 0
    while k < max_repair:
        code_target = []
        pred_target = []
        fix_idx = []
        for idx, c in enumerate(check_results):
            if c == True:
                code_target.append(iter_code[idx])
                pred_target.append(iter_pred[idx])
                fix_idx.append(idx)
        
        check_prompt = check_comment_prompt(code_target, pred_target)
        responses = vLLMClientSingleton.get_response(
        ...
        for idx, r in enumerate(responses):
            if "Yes" in r.outputs[0].text:
                check_results[fix_idx[idx]] = False
                token_usage[fix_idx[idx]] += len(r.prompt_token_ids)
            else:
                fix_times[fix_idx[idx]] += 1
                token_usage[fix_idx[idx]] += len(r.prompt_token_ids)
        
        if all(not c for c in check_results):
            break
        ...
        for idx, c in enumerate(check_results):
            if c == True:
                prompt_target.append(iter_prompt[idx])
                pred_target.append(iter_pred[idx])
                fix_idx.append(idx)
        
        fix_prompt = regen_demo_comment_prompt(prompt_target, pred_target)
        responses, line_indexs = get_osmodel_result(fix_prompt, line_indexs, templates, mode, model, n, temperture, ts)
        
        for idx, r in enumerate(responses):
            iter_pred[fix_idx[idx]] = r["best_repair"]
            token_usage[fix_idx[idx]] += r["token_usage"]
    ...
    for i in range(len(prompt)):
        final_results.append({
            "index": str(line_indexs[i]),
            "prompt": prompt[i],
            "comment": ts[i].nl,
            "code": ts[i].code,
            "best_repair": iter_pred[i],
            "best_ppl": results[i]["best_ppl"],
            "fix_times": fix_times[i],
            "token_usage": token_usage[i]
        })
    
    return final_results, line_indexs
...
class ModelTester():
        ...
        if "bm_catch_neighbor" not in locals():
            ...
            for index in tqdm(range(len(datasets))):
                ...
                if dataset_type == "valid":
                    ...
                    sim_top_n = [x[0] for x in top_retrive_results]

                    bottom_retrive_results = retrive_results[:-81:-1]
                    results_bottom_n = [x[1] for x in bottom_retrive_results]
                    with jsonlines.open(bm_catch_name, mode='a') as writer:
                        writer.write({"index": str(index), "results_top_n": results_top_n, "sim_top_n":sim_top_n ,"results_bottom_n": results_bottom_n})
    ...
    def priorization(self, method = "OFTOR"):
        ...
        if method == "OFTOR":
            ...
            # test_neighor_sim = self.metric_2("test")
            # test_neighor_sim = self.metric_2("valid")
    ...
    def prompt_chat(self,
        ...
        for line_index, prompt, t in tqdm(
            ...
            if(arrived == batch_thread):
                results, line_indexs = agent_loop(prompt_list, line_indexs, templates, mode, model, n, temperture, ts)
                if rank == True:
...
if __name__ == "__main__":
    ...
    # chat_model = "Phind/Phind-CodeLlama-34B-v2"
