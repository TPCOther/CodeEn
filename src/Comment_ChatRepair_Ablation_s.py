...
class ModelTester():
    ...
    def generate_final_result(self, model, mode, prediction_mode = "combined", classifier = None, **kwargs):
        ...
        if classifier is not None:
            output_file_name = output_file_name.replace("v2", classifier)
    ...
    def priorization(self, method = "OFTOR", classifier = "GBDT", ablation = "ALL"):
        '''
        ...
        if method == "OFTOR":
            ...
            if ablation == "ALL":
                ...
                XT = np.vstack((test_ppls, test_neighor_sim, test_sem_sim)).T
            elif ablation == "PPL":
                X = np.vstack(valid_ppls)
                XT = np.vstack(test_ppls)
            elif ablation == "STOS":
                X = np.vstack(valid_neighor_sim)
                XT = np.vstack(test_neighor_sim)
            elif ablation == "SMOS":
                X = np.vstack(valid_sem_sim)
                XT = np.vstack(test_sem_sim)
            print(X.shape)
            ...
            if classifier == "GBDT":
                ...
                clf = SVC(kernel='rbf', C=1.0, gamma='scale', probability=True).fit(X, y)
            #基于构建的clf去预测测试集
            ...
            with timer:
                wrong_prob= clf.predict_proba(XT)[:, 1]
...
if __name__ == "__main__":
    ...
    for model_under_test, abstract in map_dic.items():
        for ablation in ["SVM", "RF", "LR", "GBDT"]:
            ...
            selected_number = experiment.priorization(method="OFTOR", classifier=ablation)
            ...
            print("pre: ", pre)
            print("recall: ", recall)
            Ablation_results.append([selected_number, pre, recall, ablation])
            ...
            for prediction_model in ["combined"]:
                experiment.generate_final_result(model=chat_model, mode=repair_mode, prediction_mode=prediction_model, classifier=ablation)
        ...
        with open(result_dir + f"{model_under_test}_classifier_Ablation_results.txt", "w") as writer:
            writer.write("The Feature Importance: " + ablation + "\n")
            writer.write("*"*50 + "\n")
