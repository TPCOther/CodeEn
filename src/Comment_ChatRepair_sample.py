...
from Comment_prompt_construction import OpenAIClientSingleton, comment_gen_demo_prompt, comment_gen_system_prompt, comment_gen_user_prompt, comment_get_zero_shot_prompt, comment_get_zero_shot_prompt_repair, comment_repair_demo_prompt, vLLMClientSingleton
...
def extractResultTemplate(text, templates):
    ...
    try:
        ...
        raise Exception(f"提取模板时发生错误: {str(e)}\n文本内容: {text}")
        ...
        try:
            ...
            for choice_index, choice in enumerate(response.choices):
                if choice.finish_reason == "stop":
                    ...
                    text = text.strip()
                    print("text: ", text)
                    result = extractResultTemplate(text, templates[mode])
...
class ModelTester():
    ...
    def generate_final_result(self, model, mode, prediction_mode = "combined", method = None, **kwargs):
        ...
        if method is not None:
            output_file_name = output_file_name.replace("v2", method)
    ...
    def gini(self, model_under_test, model, tokenizer, device, select_number):
        outputs = []
        output, seq_len = get_model_output(self.test_set, self.test_objects, model, tokenizer, device, model_under_test, task="JCSD")
        for o, seql in zip(output, seq_len):
            o = o ** 2
            print(len(o), len(seql))
            for j in range(o.shape[0]):
                osum = 1 - o[j][:seql[j]].sum(dim=-1)
                sl = min(seql[j]-1, osum.shape[0]-1)
                outputs.append(osum[sl].item())
        outputs = np.array(outputs)
        indexs = np.argsort(outputs)
        return indexs[-select_number:]
    
    def BADGE_sample(self, hidden_state, model_under_test, model, tokenizer, device, select_number):
        ...
        selected_index = sampling_module.select_batch_to_label(np.array([], dtype=np.int64), select_number, task="JCSD")
    ...
    def priorization(self, method = "OFTOR"):
        ...
        if method != "PPL":
            ...
            if self.model_under_test == "CodeT5":
                select_num = 6865
                tokenizer = AutoTokenizer.from_pretrained("./outputs/JCSD/CodeT5/checkpoint-43570", device_map=device)
                model = T5ForConditionalGeneration.from_pretrained("./outputs/JCSD/CodeT5/checkpoint-43570").to(device)

            elif self.model_under_test == 'CodeGen':
                select_num = 5787
                tokenizer = AutoTokenizer.from_pretrained("./outputs/JCSD/CodeGen/checkpoint-26142",
                                                        padding_side="left", device_map=device)
                model = AutoModelForCausalLM.from_pretrained("./outputs/JCSD/CodeGen/checkpoint-26142").to(device)
                ...
                model.load_state_dict(torch.load(os.path.join("./outputs/JCSD/Unixcoder/checkpoint-4/pytorch_model.bin")),
                                    strict=False)
                model.to(device)
            ...
            if method == "badge":
                if(os.path.exists(sorted_index_desc_select_path.replace("temp", "badge")) == False):
                    with priorization_timer:
                        sorted_index_desc_select = self.BADGE_sample(test_hidden_states, model_under_test, model, tokenizer, device, select_num)
            ...
            if method == "ori":
                with priorization_timer:
                    output, seq_len, sequences = get_model_output(self.test_set, self.test_objects, model, tokenizer, device, model_under_test, return_sequences=True, task="JCSD")
...
if __name__ == "__main__":
    ...
    argparser.add_argument("--demo_num", type=int, default=5)
    ...
    demonstration_number = args.demo_num
    BEAM_SIZE = 10
    seed = 42
    setseed(seed)
    map_dic = {"CodeT5": "t5", "CodeGen": "codegen", "Unixcoder": "unix"}
    method = "gini"
    result_dir = "./outputs/JCSD_baseline/"
    # map_dic = {"CodeT5": "t5"}
    ...
    for model_under_test, abstract in map_dic.items():
        for method in ["gini"]:
            ...
            experiment.priorization(method=method)
